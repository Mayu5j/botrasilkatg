...
ИЗМЕНЕНИЯ (именованный пул прокси):
  - Новая модель Proxy — прокси больше не хранятся "россыпью" в коде,
    только в БД, с человекочитаемым именем и типом пула:
      kind="user"   — для пользовательских аккаунтов (подбирается
                      автоматически при добавлении аккаунта пользователем,
                      сам пользователь прокси не видит и не выбирает)
      kind="system" — для системных аккаунтов (админ выбирает вручную
                      самым первым шагом при добавлении системного акка)
  - Account.proxy_id / Account.proxy — связь с Proxy.
  - Старые голые поля Account.proxy_host/port/type/user/pass ОСТАВЛЕНЫ
    как есть (не удаляю рабочий код без необходимости) и используются
    как fallback в account_service.make_client(), если у аккаунта нет
    привязки к Proxy (например, аккаунты, созданные до этого рефакторинга).
"""
...
class PaymentBot(Base):
    ...
    Идея: Stars всегда падают на баланс того бота, который выставил
    счёт (это особенность Bot API, изменить нельзя). Поэтому вместо
    того чтобы слать инвойс из главного бота / зеркала, мы шлём
    пользователя по deep-link'у в отдельного "одноразового" бота,
...
# ─────────────────────────────────────────────────────────────────────────────
# ПРОКСИ (именованный пул, задаётся админом)
# ─────────────────────────────────────────────────────────────────────────────
 
class Proxy(Base):
    """
    Именованный прокси из пула. Ничего, кроме подключения к прокси-серверу,
    здесь не хранится — учётные данные Telethon остаются в Account.
 
    kind:
      "user"   — для пользовательских аккаунтов. Через прокси этого типа
                 идёт ВЕСЬ трафик обычного пользователя: отправка кода
                 при добавлении аккаунта и вся дальнейшая рассылка.
                 Подбирается автоматически (round-robin по наименее
                 загруженному активному прокси) в момент добавления
                 аккаунта — пользователь этого не видит и не выбирает.
      "system" — для системных аккаунтов. Админ выбирает прокси вручную,
                 самым первым шагом при добавлении системного аккаунта
                 (через какой прокси слать код входа и делать рассылку).
 
    accounts_count — сколько аккаунтов сейчас привязано к этому прокси.
    Используется только для round-robin при ВЫБОРЕ прокси для НОВОГО
    аккаунта. После привязки прокси у аккаунта не меняется сам по себе.
    """
    __tablename__ = "proxies"
 
    id:   Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    name: Mapped[str] = mapped_column(String(64))
    kind: Mapped[str] = mapped_column(String(16))  # "user" | "system"
    ...
    def kind_label(self) -> str:
        return "пользователи" if self.kind == "user" else "системные"
...
class Account(Base):
    ...
    Прокси:
      - proxy_id / proxy — привязка к именованному прокси из пула Proxy
        (см. модель Proxy). Это ОСНОВНОЙ механизм с этого рефакторинга.
      - proxy_host / proxy_port / proxy_type ("socks5"|"http") /
        proxy_user / proxy_pass — legacy-поля, оставлены для обратной
        совместимости со старыми аккаунтами, у которых нет proxy_id.
        account_service.make_client() использует их как fallback.
    ...
    restriction_strikes: Mapped[int] = mapped_column(Integer, default=0)
 
    # ── Прокси из именованного пула (актуальный механизм) ────────────────────
    ...
    # ── Legacy прокси-поля (fallback, если proxy_id не задан) ────────────────
