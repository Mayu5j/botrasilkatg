...
ИЗМЕНЕНИЯ (именованный пул прокси):
  - При добавлении пользователем своего аккаунта прокси подбирается
    АВТОМАТИЧЕСКИ из пула kind="user" (services.proxy_service.pick_user_proxy)
    и молча закрепляется за аккаунтом. Пользователь ничего не вводит и не
    видит упоминаний прокси — весь процесс происходит внутри got_phone /
    _finish_add_account.
  - Код входа (send_code) тоже отправляется через этот прокси — чтобы IP
    VPS не светился даже на этапе логина, как и в дальнейшей рассылке.
 
Шаги добавления аккаунта:
  1. api_id
  2. api_hash
  3. номер телефона → подбираем прокси из пула → отправляем код через Telethon
  4. код из Telegram
  5. (если 2FA) пароль
"""
import logging
...
from services import account_service, proxy_service
...
async def view_account(query: CallbackQuery, user: User, db: AsyncSession):
    ...
    await query.message.answer(text, reply_markup=kb_account_detail(acc), parse_mode="Markdown")
...
async def got_phone(message: Message, state: FSMContext, db: AsyncSession):
    phone = message.text.strip()
    data = await state.get_data()
 
    # Прокси подбирается автоматически из пула для пользователей —
    # пользователь этого не видит и никак не выбирает.
    proxy = await proxy_service.pick_user_proxy(db)
    proxy_id = proxy.id if proxy else None
    if proxy_id is None:
        log.warning("Пул прокси для пользователей пуст — аккаунт %s будет добавлен без прокси", phone)
 
    await message.answer(f"📨 Отправляю код на {phone}...")
    try:
        client, phone_code_hash = await account_service.send_code(
            data["api_id"], data["api_hash"], phone, proxy_id=proxy_id,
        )
...
async def _finish_add_account(message, state, user, db, client, phone, session_str):
    ...
    log.info("Пользователь %d добавил аккаунт %s (proxy_id=%s)", user.id, phone, data.get("proxy_id"))
